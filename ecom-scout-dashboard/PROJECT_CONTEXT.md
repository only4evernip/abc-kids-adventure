...
- 专用工作台增强（行级高亮 + 工作流捷径）
- Scout Card 一等公民化（导入 / 展示 / 单条导出）
- Feishu Bitable live sync 幂等落表（create / update）
- Feishu 同步防呆（字段缺失预检 + 重复 Card ID fail-fast）
- 批量导出同步 JSON + 批量同步脚本兼容
- 待处理队列筛选 + 保存并处理下一条
...
## 阶段判断
当前项目已经不是“规则包 + 原型 UI”，而是一个 **V1 闭环已完成** 的本地工作台。
...
5. **更细的导入恢复语义**
6. **更多专用工作流视图**
7. **Feishu 多表联动 / 更深的操盘流转**（先放规划池，不立即做）
