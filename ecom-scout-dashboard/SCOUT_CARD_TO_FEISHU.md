...
今天已经把这条链在真实 Feishu Bitable 上打通，顺手确认了几条必须写死在规则里的铁律：

### 铁律 1：DateTime 只认 13 位毫秒时间戳
- Feishu Bitable API 写 `DateTime` 字段时，**不要传 ISO 8601 字符串**
- 要传 `Number` 类型的 13 位毫秒时间戳
- 例如：`1773279000000`

### 铁律 2：字段名要以真实 Bitable 列名为准
- 本地内部对象可以用英文键
- 但真正发给 Feishu 的 payload，必须映射成目标表里的真实字段名
- 本项目当前真实写入用的是：
  - `Card ID`
  - `关键词`
  - `产品方向`
...
### 铁律 3：早期不要和 `records/search` 死磕
- Feishu `records/search` 的 filter 语法脆弱、报错含糊
- 当前阶段数据量不大时，优先方案是：
  1. `GET /records?page_size=500`
  2. 本地分页拉全量
  3. 本地比对 `Card ID`
- 先把 create/update 主链路跑通，比和第三方黑盒 filter 较劲更值钱

### 铁律 4：本地脚本要先做防呆，再做写入
当前已落地两层防护：

1. **字段缺失预检**
   - sync 前先调用 fields API
   - 拿 `Object.keys(payload)` 和真实表字段做差集
   - 有缺列就直接报错退出

2. **重复 Card ID fail-fast**
   - 如果本地查重命中多条记录
   - 直接报错并打印所有重复 `record_id`
   - **宁可不写，绝不乱写**

### 铁律 5：前端映射和 runtime 映射必须同步维护
这次联调里踩到一个很隐蔽的坑：
- `src/lib/scoutCardFeishu.ts` 改对了
- 但 `scripts/scoutCardRuntime.mjs` 还是旧映射
- 结果前端看起来对了，真实 live sync 仍然失败

结论：
> 只要存在“前端 helper + 本地脚本 helper”两套映射，就必须把它们当成同一份契约维护。
